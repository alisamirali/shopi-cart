          ...
          <div>
            <div className="text-xs flex items-center gap-5 my-7">
              <div className="flex items-center relative group cursor-pointer">
                {blog?.blogcategories?.map((item: any) => (
                  <p
                    key={item?.title}
      ...
      <div className="border border-lightColor p-5 rounded-md">
        ...
        <div className="space-y-2 mt-2">
          {categories?.map(({ blogcategories }) => (
            <div
              key={blogcategories?.[0]?.title ?? "unknown-category"}
              className="text-lightColor flex items-center justify-between text-sm font-medium"
            >
              <p>{blogcategories?.[0]?.title ?? "unknown-category"}</p>
            ...
            >
              {blog?.mainImage && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-16 h-10 rounded-md object-cover border-[1px] border-shop_dark_green/10 group-hover:border-shop_dark_green hoverEffect"
                />
              )}
              <p
                className="line-clamp-2 text-sm text-lightColor group-hover:text-shop_dark_green hoverEffect"
                title={blog?.title}
              >
                {blog?.title}
              </p>
